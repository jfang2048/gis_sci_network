...
from dashboard_data_access import load_metadata
from dashboard_data_access import require_table as require_snapshot_table
...
def require_table(name: str, *, columns: set[str] | None = None) -> pd.DataFrame:
    """Bind required-table access to the checked-in dashboard data directory."""
    return require_snapshot_table(DATA, name, columns=columns)
...
if view == "School Finder":
    render_school_finder(data_dir=DATA)

elif view == "School Profile":
    render_school_profile(data_dir=DATA, metadata=metadata, corpus=corpus)

elif view == "Compare Schools":
    render_school_comparison(data_dir=DATA, metadata=metadata, corpus=corpus)
