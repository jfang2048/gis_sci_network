...
_STAGE_VERSION = "annual-region-trends-2026-08-05-v1"
...
def build_annual_trends(
    ...
    *,
    ...
    graphs = Path(graph_metrics_path)
    for path in (flows, graphs):
        if not path.is_file():
            raise ValueError(f"trend input does not exist: {path}")
    ...
    try:
        ...
        broad_rows = connection.execute(
            ...
            WHERE corpus_view = 'broad' AND hierarchy_view = 'organization'
              AND (
                (source_region = 'Europe' AND target_region IN ('Asia', 'Americas'))
                OR (source_region = 'Americas' AND target_region = 'Asia')
                OR (
                    source_region = target_region
                    AND source_region IN ('Europe', 'Asia', 'Americas')
                )
              )
        ...
        "logical_input_hash": semantic_hash(
            {
                ...
                "graph_metrics_sha256": file_sha256(graphs),
            }
        ),
...
def _write_svg(
    ...
    *,
    ...
    parts = [
        ...
        (
            f'<text x="{left}" y="42" font-family="sans-serif" font-size="25" '
            f'font-weight="700">{html.escape(title)}</text>'
        ),
        (
            f'<text x="{left}" y="72" font-family="sans-serif" font-size="15" '
    ...
    for year in years:
        ...
        parts.append(
            ...
            f'font-family="sans-serif" font-size="12" fill="#475569">{year}</text>'
        )
    ...
    for index, series in enumerate(series_names):
        ...
        legend_x = left + (index % 3) * 340
        legend_y = height - 38 - (index // 3) * 24
        parts.append(
        ...
        parts.append(
            ...
            f'font-family="sans-serif" font-size="13">{html.escape(series)}</text>'
        )
    parts.append(
